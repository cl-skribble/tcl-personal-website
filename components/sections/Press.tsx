...
const features = [
  { outlet: "Bangkok Post",         year: "2025", headline: "The Future of Marketing Agencies Lies in Collaboration, Not Competition",         excerpt: "Chan Leong's vision for borderless marketing in Southeast Asia — building SAMA as a force for regional agency collaboration." },
  { outlet: "The Edge Malaysia",    year: "2024", headline: "Gen AI Setting New Job Standards for Creative Industry",                           excerpt: "How Skribble Group is navigating the wave of AI-driven transformation in Malaysia's creative and marketing sector." },
  { outlet: "BFM 89.9",            year: "2024", headline: "The Strategic Asia Marketing Alliance — Where Rivals and Partners Unite",          excerpt: "Marketing Mojo features Chan Leong on how SAMA is redefining the relationship between competing agencies in the region." },
  { outlet: "Enterprise Zone",      year: "2025", headline: "Chan Leong Teng — CEO of Skribble Malaysia & Founder of SAMA",                   excerpt: "An in-depth profile on CL's journey from corporate digital transformation leader to building one of SEA's most innovative marketing groups." },
];
      ...
      <div className="flex flex-col items-center gap-0.5 transition-all duration-300 grayscale opacity-30 group-hover:grayscale-0 group-hover:opacity-85">
        ...
        {outlet.sub && (
          <span className="text-[9px] font-semibold tracking-widest uppercase leading-none"
            style={{ color: outlet.color, opacity: 0.7 }}>{outlet.sub}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <section id="press" className="py-24 lg:py-32 bg-warm-white blueprint-grid-fine">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {features.map((f, i) => (
            <FadeIn key={f.headline} delay={i * 0.1}>
              <div className="border border-slate-gray/12 rounded-2xl p-7 hover:border-catalyst-blue/35 hover:shadow-sm transition-all group bg-white">
                <div className="flex items-center gap-3 mb-4">
                  <span className="bg-warm-white text-slate-gray text-xs font-semibold px-3 py-1 rounded-full border border-slate-gray/12">{f.outlet}</span>
                  <span className="text-slate-gray/45 text-xs">{f.year}</span>
                </div>
        ...
        {/* Coverage timeline */}
        <FadeIn delay={0.3}>
          <div className="mt-12 bg-white border border-slate-gray/12 rounded-2xl p-7">
